    ...
    def MFRC522_DumpClassic1K(self,uid, Start=0, End=64, keyA=None, keyB=None):
        for absoluteBlock in range(Start,End):
            ...
            if True:#status == self.OK:
                ...
                break
        if status == self.ERR:
            print("SANITY Authentication error")
            return self.ERR
        return self.OK
...
try:
    while True:
        ...
        print("CARD ID (bytes): "+str(uid))

        card = int.from_bytes(bytes(uid),"little",False)
        print("CARD ID (little): "+str(card))

        card = int.from_bytes(bytes(uid),"big",False)
        print("CARD ID (big): "+str(card))
